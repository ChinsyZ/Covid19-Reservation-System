...
{	
	...
	addr_serv.sin_port = htons(PORTc);                  //port #
	...
	while(1){
		...
		if(ava>1||ava<0){
			printf("Hospital C has None availability\n");	
		}
		...
		if(!r_loc){
			...
			if(ava>=0 && ava <+1){
				cout << "Hospital C has capacity = " << cap_total << ",occupation= " << occupation <<", availability = " << ava <<endl;
			}
			dis = path_dis(result,a,s);
			float score;
			if(dis != 0){
				printf("Hospital C has found the shortest path to client,distance = %.2f\n",dis);
				//calulate
				score =(float) 1/(dis*ava);
				printf("Hospital C has the score = %.4f\n",score);
			}
			...
			if(dis==0){
				printf("Hospital C has sent score = None and distance = None to the Scheduler\n");				
			}else{
				printf("Hospital C has sent score = %.4f and distance= %.2f to the Scheduler\n",score,dis);
			}
            //receive assignment
            //Reference：Beej's
			...
			occupation += 1;
			ava = (float) (cap_total - occupation)/cap_total;
			cout << "Hospital C has been assigned to a client, occupation is updated to " << cap_total << ",occupation is updated to " << occupation <<", availability = " << ava <<endl;	
		}
        // block
		recv_num = recvfrom(udp_sockfd,recv_buf,sizeof(recv_buf),0,(struct sockaddr *)&addr_client,(socklen_t *)&len);
	}
